...
description: 'List all [LiminalScenario] declared in the running Unity project via `lp scenarios`. Use to pick a named scenario before invoking lp-run-scenario, show stepCount and description, or detect VContainer mis-registration via stepCount=-1.'
...
LiminalPalette に `[LiminalScenario]` で宣言されたシナリオの一覧を `lp scenarios` で取得する。`lp-run-scenario` で named 実行する前の発見ステップ。

シナリオは「複数ステップ (command / wait / assert) を順次実行する宣言」で、`[LiminalCommand]` の集合体に近い。詳細は `/lp-run-scenario` を参照。
...
- `/lp-list-commands` — シナリオ内 `command` ステップで使う `[LiminalCommand]` の発見
- `/lp-get-state` — シナリオ内 `assert_equals` で使う `[LiminalObservableField]` の現在値
