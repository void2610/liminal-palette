...
#
# 通常運用は `lp --port 7610 ...` / `lp --port 7611 ...` を直接使えば足りる。
...
for p in 7610 7611 7612 7613 7614 7615; do
    resp=$(lp --port "$p" --json health 2>/dev/null) || continue
    [ -n "$resp" ] || continue
...
*)
    ...
    echo ""
    echo "使用例:"
    echo "  lp --port \$LP_PORT_EDITOR exec Editor/Console/Clear"
    echo "  lp --port \$LP_PORT_RUNTIME exec Player/Health/Set value=100"
    ;;
esac
